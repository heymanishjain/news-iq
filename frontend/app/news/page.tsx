  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      ...
      <div className="sticky top-0 z-50 border-b border-gray-200/80 dark:border-gray-800/80 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl shadow-sm">
        <div className="mx-auto max-w-7xl px-4 sm:px-4 lg:px-6 py-2">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700 transition-all duration-200 border-2 border-transparent hover:border-gray-300 dark:hover:border-gray-600"
              aria-label={isFiltersExpanded ? "Collapse filters" : "Expand filters"}
            >
              <span className="text-sm font-semibold">{isFiltersExpanded ? "Hide" : "Show"} Filters</span>
              <svg
            ...
            {Object.entries(groupedArticles).map(([dateGroup, dateArticles]) => (
              <div key={dateGroup} className="space-y-6">
                <div className="flex items-center gap-3">
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{dateGroup}</h2>
                  <div className="flex-1 h-px bg-gradient-to-r from-gray-300 to-transparent dark:from-gray-700"></div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">{dateArticles.length} stories</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {dateArticles.map((article, index) => {
                    ...
                    return (
                      <article
                        key={article.id}
                        className={`group relative bg-white dark:bg-gray-800/50 rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-800/50 hover:border-gray-300 dark:hover:border-gray-700 transition-all duration-300 hover:shadow-xl hover:shadow-gray-200/50 dark:hover:shadow-gray-900/50 ${
                      ...
                      >
                        ...
                        >
                          {/* Image Section */}
                          <div className={`relative ${isLarge ? "h-72" : "h-56"} bg-gradient-to-br ${categoryConfig.bg} overflow-hidden`}>
                            {hasImage ? (
                              <>
                                <img
                          ...
                          <div className="p-6">
                            <div className="flex items-center gap-2 mb-3 flex-wrap">
                              <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg ${sourceConfig.badge} border border-current/20`}>
                                <span className="text-xs">{sourceConfig.icon}</span>
                            ...
                            <h3 className={`font-bold text-gray-900 dark:text-white mb-3 group-hover:text-primary transition-colors ${
                              isLarge ? "text-2xl leading-tight line-clamp-3" : "text-xl leading-snug line-clamp-2"
                            }`}>
                              {article.title}
                            </h3>
                            ...
                            <div className="flex items-center text-primary dark:text-blue-400 text-sm font-semibold group-hover:gap-2 transition-all">
                              Read full story
                              <svg
